...
def get_class_weights(train_df):
    ...
    #balancing individual calss importance
...
def main():
    init_results_dir()
    LOG_FILE = "results/bert_log.txt"
    METRICS_FILE = "results/bert_metrics.csv"
    ...
    for epoch in range(epochs):
        ...
        save_confusion_matrix(val_result["confusion_matrix"], "results/bert_confusion_val.csv")
        save_report(val_result["report"], "results/bert_report_val.txt")
        ...
        if val_f1 > best_f1:
            ...
            log_to_file("Saved best model!", LOG_FILE)

    print("\nFinal Test (CON):")

    test_result = evaluate(model, test_loader, threshold=0.5, name="Test (CON)")

    log_to_file(f"\nTest Results:\n{test_result}", LOG_FILE)
    save_metrics(test_result, METRICS_FILE)
    save_confusion_matrix(test_result["confusion_matrix"], "results/bert_confusion_test.csv")
    save_report(test_result["report"], "results/bert_report_test.txt")
